...
public class ConfigManager {
    ...
    /**
     * Saves the YAML configuration file and reloads its contents.
     * Note that this method DOES NOT save comments below the header comment in the file.
     * As a result, this method should not be called on config files it should only be called
     * on data files that do not need comments.
     */
    public void save() {
        try {
    ...
    /**
     * Reloads the YAML configuration file.
     * Keeps Comments.
     */
    public void reload() {
    ...
    /**
     * Get a string from a node in the configuration file.
     * Parses '&' color codes automatically.
     *
    ...
    /**
     * Get a string from a node in the configuration file.
     * Allows the declaration of placeholders and a string to replace them with. Placeholders are replaced using the fast {@link StringUtils} class.
     * Parses '&' color codes automatically.
     * @param node The configuration node to search
    ...
    public String getString(String node, Map<String, String> placeHolders) {
        return StringUtils.replaceEachRepeatedly(getString(node), placeHolders.keySet().toArray(new String[0]), placeHolders.values().toArray(new String[0]));
    }
    ...
    public float getFloat(String node) {
        try {
            ...
            plugin.getLogger().severe("CONFIG EXCEPTION: Float value at node \"" + node + "\" is not a valid floating point value. (Bad Type Cast)");
            return -1;
        }
    }
    ...
    /**
     * Set a specific value in the configuration file given a node and a value.
     * This method "saves" the configuration file but does NOT reload it.
     *

...
public class LootPage extends BasePage {
	...
	public ItemStack[] getContent() {
		...
		dropViewMeta.setDisplayName(ChatColor.GRAY + "Viewing Drop " + ChatColor.DARK_GRAY + "-> " + ChatColor.DARK_AQUA + (getDropIndex()));
		dropViewMeta.setLore(Arrays.asList(
				ChatColor.GRAY + "You are currently viewing all of the loot",
				ChatColor.GRAY + "that can drop from this drop.",
				ChatColor.DARK_GRAY + "- " + ChatColor.GRAY + "Rarity: " + ChatColor.BLUE + plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getRarity(),
				ChatColor.DARK_GRAY + "- " + ChatColor.GRAY + "# of Items: " + ChatColor.BLUE + plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getItems().size(),
				ChatColor.DARK_GRAY + "- " + ChatColor.GRAY + "# of Potions: " + ChatColor.BLUE + plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getPotionEffects().size(),
				ChatColor.DARK_GRAY + "- " + ChatColor.GRAY + "# of Commands: " + ChatColor.BLUE + plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getCommands().size(),
        ...
        for(int i = 0; i < 4; i++) content[2][i] = new ItemStack(Material.RED_STAINED_GLASS_PANE);
        ...
        ArrayList<LootProperty> dropList = plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getLoot();
	...
	public void onClick(MenuView view, ClickType clickType, int slot, ItemStack itemStack) {
		...
		if(itemStack.equals(getSpecialItem(SpecialItem.COPY_DROP))) {
			final LuckyBlockDrop drop = plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex());
			plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().add(new LuckyBlockDrop(drop.getItems(), drop.getCommands(), drop.getPotionEffects(), drop.getRarity()));

...
public class EditDrop extends BasePage {
	...
	private boolean canAddItems() {
		return plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getLoot()
				.size() < LuckyBlockDrop.MAX_ALLOWED_LOOT;
	}
	...
	public ItemStack[] getContent() {
		...
		meta.setDisplayName(
				ChatColor.GRAY + "Editing Drop " + ChatColor.DARK_GRAY + "-> " + ChatColor.DARK_AQUA + getDropIndex());
		...
		ArrayList<LootProperty> lootList = plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems()
	...
	public void onClick(MenuView view, ClickType clickType, int slot, ItemStack itemStack) {
			...
			if (isPlayerSlot(slot)) {
				if (!canAddItems()) {
					view.getPlayer().sendMessage(ChatColor.RED + "You are only allowed to add up to "
							+ LuckyBlockDrop.MAX_ALLOWED_LOOT + " items per drop.");
					return;
				}
				plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getItems()
						.add(new LuckyBlockItem(itemStack));
			} else {
				if (!(slot >= 2 && slot <= 8 || slot >= 11 && slot <= 17) || (plugin.getLuckyBlocks()
						.get(getBlockIndex()).getDroppableItems().get(getDropIndex()).getLoot().size() == 1))
					return;
				plugin.getLuckyBlocks().get(getBlockIndex()).getDroppableItems().get(getDropIndex())
		...
		if (itemStack.equals(getSpecialItem(SpecialItem.ADD_COMMAND))) {
			...
			view.getPlayer().sendMessage(ChatColor.GRAY + "You are attempting to add a command to drop: "
					+ ChatColor.BLUE + String.valueOf(getDropIndex()));

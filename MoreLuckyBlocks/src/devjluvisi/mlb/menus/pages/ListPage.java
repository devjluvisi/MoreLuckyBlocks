...
public class ListPage extends BasePage {
	...
	public ItemStack[] getContent() {
		...
		for (int i = 0; i < 9; i++)
			content[1][i] = new ItemCreator(randomPane()).setName("").getItem();
		for (int i = 0; i < 9; i++)
			content[4][i] = new ItemCreator(randomPane()).setName("").getItem();
			...
			for (int j = 1; j < 9 && luckyBlockIndex != plugin.getLuckyBlocks().size(); j++) {
				...
				meta.setLore(Arrays.asList(
						ChatColor.DARK_AQUA + "Item Name" + ChatColor.GRAY + ": "
								+ plugin.getLuckyBlocks().get(luckyBlockIndex).getName(),
						ChatColor.DARK_AQUA + "Break Permission" + ChatColor.GRAY + ": "
								+ plugin.getLuckyBlocks().get(luckyBlockIndex).getBreakPermission(),
						ChatColor.DARK_AQUA + "Material" + ChatColor.GRAY + ": "
								+ plugin.getLuckyBlocks().get(luckyBlockIndex).getBlockMaterial().name(),
						ChatColor.DARK_AQUA + "Default Luck" + ChatColor.GRAY + ": "
								+ plugin.getLuckyBlocks().get(luckyBlockIndex).getDefaultBlockLuck(),
						ChatColor.DARK_AQUA + "# of Droppable Items" + ChatColor.GRAY + ": "
								+ plugin.getLuckyBlocks().get(luckyBlockIndex).getDroppableItems().size(),
						ChatColor.DARK_AQUA + "Lore Length" + ChatColor.GRAY + ": "
								+ plugin.getLuckyBlocks().get(luckyBlockIndex).getRefreshedLore().size(),
	...
	private Material randomPane() {
		Material[] glassPanes = new Material[] { Material.BLUE_STAINED_GLASS_PANE, Material.BROWN_STAINED_GLASS_PANE,
	...
	public void onClick(MenuView view, ClickType clickType, int slot, ItemStack item) {
		...
		for (int i = 0; i < plugin.getLuckyBlocks().size(); i++) {
			if (ChatColor.stripColor(item.getItemMeta().getDisplayName())
					.equalsIgnoreCase(ChatColor.stripColor(plugin.getLuckyBlocks().get(i).getInternalName()))) {

...
public class ListPage extends BasePage {
    ...
    public ItemStack[] getContent() {
        ...
        // Fill empty slots with lucky blocks.
        for (int i = 2; i != 4; i++) {
        ...
        // Fill slots where there are no lucky blocks to put.
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 9; j++) {
                if (content[i][j] == null) {
    ...
    public void onClick(MenuView view, ClickType clickType, int slot, ItemStack item) {
        ...
        final int index = this.plugin.getLuckyBlocks()
                .indexOf(new LuckyBlock(Util.makeInternal(Objects.requireNonNull(item.getItemMeta()).getDisplayName())));
        if (index == -1) {
            return;
        }
